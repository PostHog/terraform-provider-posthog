...
// shares it. What only this layer shows is that the adapter reaches those rules from this
// resource's nested shape, and how it handles element shapes core never sees.
func TestBlockedWindowsValidator(t *testing.T) {
	...
	t.Run("null and unknown are left alone", func(t *testing.T) {
		for name, value := range map[string]types.Set{
			"null set":    types.SetNull(alertBlockedWindowObjectType),
			"unknown set": types.SetUnknown(alertBlockedWindowObjectType),
		} {
