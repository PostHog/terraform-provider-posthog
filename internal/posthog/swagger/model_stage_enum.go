...
// StageEnum * `draft` - draft * `concept` - concept * `alpha` - alpha * `beta` - beta * `general-availability` - general availability * `archived` - archived
type StageEnum string

// List of StageEnum
const (
	DRAFT StageEnum = "draft"
	CONCEPT StageEnum = "concept"
	ALPHA StageEnum = "alpha"
	BETA StageEnum = "beta"
	GENERAL_AVAILABILITY StageEnum = "general-availability"
	ARCHIVED StageEnum = "archived"
)
...
func NewStageEnumFromValue(v string) (*StageEnum, error) {
	ev := StageEnum(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
