/*
PostHog API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package posthogapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the ExperimentFunnelMetric type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ExperimentFunnelMetric{}

// ExperimentFunnelMetric struct for ExperimentFunnelMetric
type ExperimentFunnelMetric struct {
	BreakdownFilter *BreakdownFilter `json:"breakdownFilter,omitempty"`
	ConversionWindow NullableInt32 `json:"conversion_window,omitempty"`
	ConversionWindowUnit *FunnelConversionWindowTimeUnit `json:"conversion_window_unit,omitempty"`
	Fingerprint NullableString `json:"fingerprint,omitempty"`
	FunnelOrderType *StepOrderValue `json:"funnel_order_type,omitempty"`
	Goal *ExperimentMetricGoal `json:"goal,omitempty"`
	IsSharedMetric NullableBool `json:"isSharedMetric,omitempty"`
	Kind *string `json:"kind,omitempty"`
	MetricType *string `json:"metric_type,omitempty"`
	Name NullableString `json:"name,omitempty"`
	Response map[string]interface{} `json:"response,omitempty"`
	Series []Series1Inner1 `json:"series"`
	SharedMetricId NullableFloat32 `json:"sharedMetricId,omitempty"`
	Uuid NullableString `json:"uuid,omitempty"`
	// version of the node, used for schema migrations
	Version NullableFloat32 `json:"version,omitempty"`
}

type _ExperimentFunnelMetric ExperimentFunnelMetric

// NewExperimentFunnelMetric instantiates a new ExperimentFunnelMetric object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewExperimentFunnelMetric(series []Series1Inner1) *ExperimentFunnelMetric {
	this := ExperimentFunnelMetric{}
	var kind string = "ExperimentMetric"
	this.Kind = &kind
	var metricType string = "funnel"
	this.MetricType = &metricType
	this.Series = series
	return &this
}

// NewExperimentFunnelMetricWithDefaults instantiates a new ExperimentFunnelMetric object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewExperimentFunnelMetricWithDefaults() *ExperimentFunnelMetric {
	this := ExperimentFunnelMetric{}
	var kind string = "ExperimentMetric"
	this.Kind = &kind
	var metricType string = "funnel"
	this.MetricType = &metricType
	return &this
}

// GetBreakdownFilter returns the BreakdownFilter field value if set, zero value otherwise.
func (o *ExperimentFunnelMetric) GetBreakdownFilter() BreakdownFilter {
	if o == nil || IsNil(o.BreakdownFilter) {
		var ret BreakdownFilter
		return ret
	}
	return *o.BreakdownFilter
}

// GetBreakdownFilterOk returns a tuple with the BreakdownFilter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ExperimentFunnelMetric) GetBreakdownFilterOk() (*BreakdownFilter, bool) {
	if o == nil || IsNil(o.BreakdownFilter) {
		return nil, false
	}
	return o.BreakdownFilter, true
}

// HasBreakdownFilter returns a boolean if a field has been set.
func (o *ExperimentFunnelMetric) HasBreakdownFilter() bool {
	if o != nil && !IsNil(o.BreakdownFilter) {
		return true
	}

	return false
}

// SetBreakdownFilter gets a reference to the given BreakdownFilter and assigns it to the BreakdownFilter field.
func (o *ExperimentFunnelMetric) SetBreakdownFilter(v BreakdownFilter) {
	o.BreakdownFilter = &v
}

// GetConversionWindow returns the ConversionWindow field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ExperimentFunnelMetric) GetConversionWindow() int32 {
	if o == nil || IsNil(o.ConversionWindow.Get()) {
		var ret int32
		return ret
	}
	return *o.ConversionWindow.Get()
}

// GetConversionWindowOk returns a tuple with the ConversionWindow field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ExperimentFunnelMetric) GetConversionWindowOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.ConversionWindow.Get(), o.ConversionWindow.IsSet()
}

// HasConversionWindow returns a boolean if a field has been set.
func (o *ExperimentFunnelMetric) HasConversionWindow() bool {
	if o != nil && o.ConversionWindow.IsSet() {
		return true
	}

	return false
}

// SetConversionWindow gets a reference to the given NullableInt32 and assigns it to the ConversionWindow field.
func (o *ExperimentFunnelMetric) SetConversionWindow(v int32) {
	o.ConversionWindow.Set(&v)
}
// SetConversionWindowNil sets the value for ConversionWindow to be an explicit nil
func (o *ExperimentFunnelMetric) SetConversionWindowNil() {
	o.ConversionWindow.Set(nil)
}

// UnsetConversionWindow ensures that no value is present for ConversionWindow, not even an explicit nil
func (o *ExperimentFunnelMetric) UnsetConversionWindow() {
	o.ConversionWindow.Unset()
}

// GetConversionWindowUnit returns the ConversionWindowUnit field value if set, zero value otherwise.
func (o *ExperimentFunnelMetric) GetConversionWindowUnit() FunnelConversionWindowTimeUnit {
	if o == nil || IsNil(o.ConversionWindowUnit) {
		var ret FunnelConversionWindowTimeUnit
		return ret
	}
	return *o.ConversionWindowUnit
}

// GetConversionWindowUnitOk returns a tuple with the ConversionWindowUnit field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ExperimentFunnelMetric) GetConversionWindowUnitOk() (*FunnelConversionWindowTimeUnit, bool) {
	if o == nil || IsNil(o.ConversionWindowUnit) {
		return nil, false
	}
	return o.ConversionWindowUnit, true
}

// HasConversionWindowUnit returns a boolean if a field has been set.
func (o *ExperimentFunnelMetric) HasConversionWindowUnit() bool {
	if o != nil && !IsNil(o.ConversionWindowUnit) {
		return true
	}

	return false
}

// SetConversionWindowUnit gets a reference to the given FunnelConversionWindowTimeUnit and assigns it to the ConversionWindowUnit field.
func (o *ExperimentFunnelMetric) SetConversionWindowUnit(v FunnelConversionWindowTimeUnit) {
	o.ConversionWindowUnit = &v
}

// GetFingerprint returns the Fingerprint field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ExperimentFunnelMetric) GetFingerprint() string {
	if o == nil || IsNil(o.Fingerprint.Get()) {
		var ret string
		return ret
	}
	return *o.Fingerprint.Get()
}

// GetFingerprintOk returns a tuple with the Fingerprint field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ExperimentFunnelMetric) GetFingerprintOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Fingerprint.Get(), o.Fingerprint.IsSet()
}

// HasFingerprint returns a boolean if a field has been set.
func (o *ExperimentFunnelMetric) HasFingerprint() bool {
	if o != nil && o.Fingerprint.IsSet() {
		return true
	}

	return false
}

// SetFingerprint gets a reference to the given NullableString and assigns it to the Fingerprint field.
func (o *ExperimentFunnelMetric) SetFingerprint(v string) {
	o.Fingerprint.Set(&v)
}
// SetFingerprintNil sets the value for Fingerprint to be an explicit nil
func (o *ExperimentFunnelMetric) SetFingerprintNil() {
	o.Fingerprint.Set(nil)
}

// UnsetFingerprint ensures that no value is present for Fingerprint, not even an explicit nil
func (o *ExperimentFunnelMetric) UnsetFingerprint() {
	o.Fingerprint.Unset()
}

// GetFunnelOrderType returns the FunnelOrderType field value if set, zero value otherwise.
func (o *ExperimentFunnelMetric) GetFunnelOrderType() StepOrderValue {
	if o == nil || IsNil(o.FunnelOrderType) {
		var ret StepOrderValue
		return ret
	}
	return *o.FunnelOrderType
}

// GetFunnelOrderTypeOk returns a tuple with the FunnelOrderType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ExperimentFunnelMetric) GetFunnelOrderTypeOk() (*StepOrderValue, bool) {
	if o == nil || IsNil(o.FunnelOrderType) {
		return nil, false
	}
	return o.FunnelOrderType, true
}

// HasFunnelOrderType returns a boolean if a field has been set.
func (o *ExperimentFunnelMetric) HasFunnelOrderType() bool {
	if o != nil && !IsNil(o.FunnelOrderType) {
		return true
	}

	return false
}

// SetFunnelOrderType gets a reference to the given StepOrderValue and assigns it to the FunnelOrderType field.
func (o *ExperimentFunnelMetric) SetFunnelOrderType(v StepOrderValue) {
	o.FunnelOrderType = &v
}

// GetGoal returns the Goal field value if set, zero value otherwise.
func (o *ExperimentFunnelMetric) GetGoal() ExperimentMetricGoal {
	if o == nil || IsNil(o.Goal) {
		var ret ExperimentMetricGoal
		return ret
	}
	return *o.Goal
}

// GetGoalOk returns a tuple with the Goal field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ExperimentFunnelMetric) GetGoalOk() (*ExperimentMetricGoal, bool) {
	if o == nil || IsNil(o.Goal) {
		return nil, false
	}
	return o.Goal, true
}

// HasGoal returns a boolean if a field has been set.
func (o *ExperimentFunnelMetric) HasGoal() bool {
	if o != nil && !IsNil(o.Goal) {
		return true
	}

	return false
}

// SetGoal gets a reference to the given ExperimentMetricGoal and assigns it to the Goal field.
func (o *ExperimentFunnelMetric) SetGoal(v ExperimentMetricGoal) {
	o.Goal = &v
}

// GetIsSharedMetric returns the IsSharedMetric field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ExperimentFunnelMetric) GetIsSharedMetric() bool {
	if o == nil || IsNil(o.IsSharedMetric.Get()) {
		var ret bool
		return ret
	}
	return *o.IsSharedMetric.Get()
}

// GetIsSharedMetricOk returns a tuple with the IsSharedMetric field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ExperimentFunnelMetric) GetIsSharedMetricOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return o.IsSharedMetric.Get(), o.IsSharedMetric.IsSet()
}

// HasIsSharedMetric returns a boolean if a field has been set.
func (o *ExperimentFunnelMetric) HasIsSharedMetric() bool {
	if o != nil && o.IsSharedMetric.IsSet() {
		return true
	}

	return false
}

// SetIsSharedMetric gets a reference to the given NullableBool and assigns it to the IsSharedMetric field.
func (o *ExperimentFunnelMetric) SetIsSharedMetric(v bool) {
	o.IsSharedMetric.Set(&v)
}
// SetIsSharedMetricNil sets the value for IsSharedMetric to be an explicit nil
func (o *ExperimentFunnelMetric) SetIsSharedMetricNil() {
	o.IsSharedMetric.Set(nil)
}

// UnsetIsSharedMetric ensures that no value is present for IsSharedMetric, not even an explicit nil
func (o *ExperimentFunnelMetric) UnsetIsSharedMetric() {
	o.IsSharedMetric.Unset()
}

// GetKind returns the Kind field value if set, zero value otherwise.
func (o *ExperimentFunnelMetric) GetKind() string {
	if o == nil || IsNil(o.Kind) {
		var ret string
		return ret
	}
	return *o.Kind
}

// GetKindOk returns a tuple with the Kind field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ExperimentFunnelMetric) GetKindOk() (*string, bool) {
	if o == nil || IsNil(o.Kind) {
		return nil, false
	}
	return o.Kind, true
}

// HasKind returns a boolean if a field has been set.
func (o *ExperimentFunnelMetric) HasKind() bool {
	if o != nil && !IsNil(o.Kind) {
		return true
	}

	return false
}

// SetKind gets a reference to the given string and assigns it to the Kind field.
func (o *ExperimentFunnelMetric) SetKind(v string) {
	o.Kind = &v
}

// GetMetricType returns the MetricType field value if set, zero value otherwise.
func (o *ExperimentFunnelMetric) GetMetricType() string {
	if o == nil || IsNil(o.MetricType) {
		var ret string
		return ret
	}
	return *o.MetricType
}

// GetMetricTypeOk returns a tuple with the MetricType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ExperimentFunnelMetric) GetMetricTypeOk() (*string, bool) {
	if o == nil || IsNil(o.MetricType) {
		return nil, false
	}
	return o.MetricType, true
}

// HasMetricType returns a boolean if a field has been set.
func (o *ExperimentFunnelMetric) HasMetricType() bool {
	if o != nil && !IsNil(o.MetricType) {
		return true
	}

	return false
}

// SetMetricType gets a reference to the given string and assigns it to the MetricType field.
func (o *ExperimentFunnelMetric) SetMetricType(v string) {
	o.MetricType = &v
}

// GetName returns the Name field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ExperimentFunnelMetric) GetName() string {
	if o == nil || IsNil(o.Name.Get()) {
		var ret string
		return ret
	}
	return *o.Name.Get()
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ExperimentFunnelMetric) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Name.Get(), o.Name.IsSet()
}

// HasName returns a boolean if a field has been set.
func (o *ExperimentFunnelMetric) HasName() bool {
	if o != nil && o.Name.IsSet() {
		return true
	}

	return false
}

// SetName gets a reference to the given NullableString and assigns it to the Name field.
func (o *ExperimentFunnelMetric) SetName(v string) {
	o.Name.Set(&v)
}
// SetNameNil sets the value for Name to be an explicit nil
func (o *ExperimentFunnelMetric) SetNameNil() {
	o.Name.Set(nil)
}

// UnsetName ensures that no value is present for Name, not even an explicit nil
func (o *ExperimentFunnelMetric) UnsetName() {
	o.Name.Unset()
}

// GetResponse returns the Response field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ExperimentFunnelMetric) GetResponse() map[string]interface{} {
	if o == nil {
		var ret map[string]interface{}
		return ret
	}
	return o.Response
}

// GetResponseOk returns a tuple with the Response field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ExperimentFunnelMetric) GetResponseOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.Response) {
		return map[string]interface{}{}, false
	}
	return o.Response, true
}

// HasResponse returns a boolean if a field has been set.
func (o *ExperimentFunnelMetric) HasResponse() bool {
	if o != nil && !IsNil(o.Response) {
		return true
	}

	return false
}

// SetResponse gets a reference to the given map[string]interface{} and assigns it to the Response field.
func (o *ExperimentFunnelMetric) SetResponse(v map[string]interface{}) {
	o.Response = v
}

// GetSeries returns the Series field value
func (o *ExperimentFunnelMetric) GetSeries() []Series1Inner1 {
	if o == nil {
		var ret []Series1Inner1
		return ret
	}

	return o.Series
}

// GetSeriesOk returns a tuple with the Series field value
// and a boolean to check if the value has been set.
func (o *ExperimentFunnelMetric) GetSeriesOk() ([]Series1Inner1, bool) {
	if o == nil {
		return nil, false
	}
	return o.Series, true
}

// SetSeries sets field value
func (o *ExperimentFunnelMetric) SetSeries(v []Series1Inner1) {
	o.Series = v
}

// GetSharedMetricId returns the SharedMetricId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ExperimentFunnelMetric) GetSharedMetricId() float32 {
	if o == nil || IsNil(o.SharedMetricId.Get()) {
		var ret float32
		return ret
	}
	return *o.SharedMetricId.Get()
}

// GetSharedMetricIdOk returns a tuple with the SharedMetricId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ExperimentFunnelMetric) GetSharedMetricIdOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return o.SharedMetricId.Get(), o.SharedMetricId.IsSet()
}

// HasSharedMetricId returns a boolean if a field has been set.
func (o *ExperimentFunnelMetric) HasSharedMetricId() bool {
	if o != nil && o.SharedMetricId.IsSet() {
		return true
	}

	return false
}

// SetSharedMetricId gets a reference to the given NullableFloat32 and assigns it to the SharedMetricId field.
func (o *ExperimentFunnelMetric) SetSharedMetricId(v float32) {
	o.SharedMetricId.Set(&v)
}
// SetSharedMetricIdNil sets the value for SharedMetricId to be an explicit nil
func (o *ExperimentFunnelMetric) SetSharedMetricIdNil() {
	o.SharedMetricId.Set(nil)
}

// UnsetSharedMetricId ensures that no value is present for SharedMetricId, not even an explicit nil
func (o *ExperimentFunnelMetric) UnsetSharedMetricId() {
	o.SharedMetricId.Unset()
}

// GetUuid returns the Uuid field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ExperimentFunnelMetric) GetUuid() string {
	if o == nil || IsNil(o.Uuid.Get()) {
		var ret string
		return ret
	}
	return *o.Uuid.Get()
}

// GetUuidOk returns a tuple with the Uuid field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ExperimentFunnelMetric) GetUuidOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Uuid.Get(), o.Uuid.IsSet()
}

// HasUuid returns a boolean if a field has been set.
func (o *ExperimentFunnelMetric) HasUuid() bool {
	if o != nil && o.Uuid.IsSet() {
		return true
	}

	return false
}

// SetUuid gets a reference to the given NullableString and assigns it to the Uuid field.
func (o *ExperimentFunnelMetric) SetUuid(v string) {
	o.Uuid.Set(&v)
}
// SetUuidNil sets the value for Uuid to be an explicit nil
func (o *ExperimentFunnelMetric) SetUuidNil() {
	o.Uuid.Set(nil)
}

// UnsetUuid ensures that no value is present for Uuid, not even an explicit nil
func (o *ExperimentFunnelMetric) UnsetUuid() {
	o.Uuid.Unset()
}

// GetVersion returns the Version field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ExperimentFunnelMetric) GetVersion() float32 {
	if o == nil || IsNil(o.Version.Get()) {
		var ret float32
		return ret
	}
	return *o.Version.Get()
}

// GetVersionOk returns a tuple with the Version field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ExperimentFunnelMetric) GetVersionOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return o.Version.Get(), o.Version.IsSet()
}

// HasVersion returns a boolean if a field has been set.
func (o *ExperimentFunnelMetric) HasVersion() bool {
	if o != nil && o.Version.IsSet() {
		return true
	}

	return false
}

// SetVersion gets a reference to the given NullableFloat32 and assigns it to the Version field.
func (o *ExperimentFunnelMetric) SetVersion(v float32) {
	o.Version.Set(&v)
}
// SetVersionNil sets the value for Version to be an explicit nil
func (o *ExperimentFunnelMetric) SetVersionNil() {
	o.Version.Set(nil)
}

// UnsetVersion ensures that no value is present for Version, not even an explicit nil
func (o *ExperimentFunnelMetric) UnsetVersion() {
	o.Version.Unset()
}

func (o ExperimentFunnelMetric) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ExperimentFunnelMetric) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.BreakdownFilter) {
		toSerialize["breakdownFilter"] = o.BreakdownFilter
	}
	if o.ConversionWindow.IsSet() {
		toSerialize["conversion_window"] = o.ConversionWindow.Get()
	}
	if !IsNil(o.ConversionWindowUnit) {
		toSerialize["conversion_window_unit"] = o.ConversionWindowUnit
	}
	if o.Fingerprint.IsSet() {
		toSerialize["fingerprint"] = o.Fingerprint.Get()
	}
	if !IsNil(o.FunnelOrderType) {
		toSerialize["funnel_order_type"] = o.FunnelOrderType
	}
	if !IsNil(o.Goal) {
		toSerialize["goal"] = o.Goal
	}
	if o.IsSharedMetric.IsSet() {
		toSerialize["isSharedMetric"] = o.IsSharedMetric.Get()
	}
	if !IsNil(o.Kind) {
		toSerialize["kind"] = o.Kind
	}
	if !IsNil(o.MetricType) {
		toSerialize["metric_type"] = o.MetricType
	}
	if o.Name.IsSet() {
		toSerialize["name"] = o.Name.Get()
	}
	if o.Response != nil {
		toSerialize["response"] = o.Response
	}
	toSerialize["series"] = o.Series
	if o.SharedMetricId.IsSet() {
		toSerialize["sharedMetricId"] = o.SharedMetricId.Get()
	}
	if o.Uuid.IsSet() {
		toSerialize["uuid"] = o.Uuid.Get()
	}
	if o.Version.IsSet() {
		toSerialize["version"] = o.Version.Get()
	}
	return toSerialize, nil
}

func (o *ExperimentFunnelMetric) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"series",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varExperimentFunnelMetric := _ExperimentFunnelMetric{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varExperimentFunnelMetric)

	if err != nil {
		return err
	}

	*o = ExperimentFunnelMetric(varExperimentFunnelMetric)

	return err
}

type NullableExperimentFunnelMetric struct {
	value *ExperimentFunnelMetric
	isSet bool
}

func (v NullableExperimentFunnelMetric) Get() *ExperimentFunnelMetric {
	return v.value
}

func (v *NullableExperimentFunnelMetric) Set(val *ExperimentFunnelMetric) {
	v.value = val
	v.isSet = true
}

func (v NullableExperimentFunnelMetric) IsSet() bool {
	return v.isSet
}

func (v *NullableExperimentFunnelMetric) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableExperimentFunnelMetric(val *ExperimentFunnelMetric) *NullableExperimentFunnelMetric {
	return &NullableExperimentFunnelMetric{value: val, isSet: true}
}

func (v NullableExperimentFunnelMetric) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableExperimentFunnelMetric) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


