...
description: |-
  ...
  The alert does report which destination types exist, so the PostHog UI can show whether an alert notifies anyone, but the provider does not surface it. Replacing the resource, notably by changing project_id, creates a new alert with no destinations attached.
